...
public class Cuenta {
    ...
    //Se resta cierta cantidad de nuestro saldo
    public void debito(BigDecimal monto) {
        BigDecimal nuevoSaldo = this.saldo.subtract(monto);
        if (nuevoSaldo.compareTo(BigDecimal.ZERO) < 0) {
            throw new DineroInsuficienteException("No tiene saldo suficiente para la operación");
        }
        this.saldo = nuevoSaldo;
    }

    //Cuando recibimos dinero de otra cuenta (nos hacen transferencia)
